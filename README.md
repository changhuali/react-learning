...
- SyncLane
  具有最高优先级, 会采用微任务的方式对其进行调度, 这类更新不可中断, 多个任务间不可中断
- InputContinuousHydrationLane InputContinuousLane DefaultHydrationLane DefaultLane
  具有较高优先级, 会采用宏任务的方式对其进行调度, 这类更新不可中断, 多个任务间可中断
- TransitionLanes
  具有较低优先级, 会采用宏任务的方式对其进行调度, 这类更新可中断, 多个任务间可中断
- RetryLanes
  TODO:
- IdleLane
  TODO:
...
1. 任务中中断: 在处理一个`Fiber`之前, 会先判断当前调度的总耗时是否低于 5ms, 若满足条件, 则处理该`Fiber`, 否则发生中断, 将退出状态设置为`RootInProgress`
   如何恢复: 若退出状态码为`RootInProgress`, 则不会进入`commit阶段`, 而是立即向`taskQueue`推入一个新任务, 这个时候会进入条件 2, 而且此时一定会触发任务间的中断, 因为当前调度的总耗时 >= 5ms, 由于刚刚向`taskQueue`推入了一个新任务, 所以如果原`taskQueue`中没有更高优先级任务的任务, 那刚被推入的任务就会在新的一次调度中被执行, 而`renderConcurrentRoot`中会判断此次任务和上次任务的优先级以及 root 是否相同, 若都相同, 说明此次任务应该从上次中断处开始执行, 所以不会调用`prepareFreshStack`重置`workInProgress`
2. 任务间中断: 在执行一个任务之前, 会先判断当前调度的总耗时是否低于 5ms 或任务是否超时, 若满足任一条件, 则执行当前任务, 否则发生中断
   如何恢复: 中断后判断`taskQueue`中是否还有待执行任务, 若有则重新发起一次调度, 此时调度的开始时间会被重置, 因此下次在判断`调度的总耗时 < 5ms`时条件一定为 true, 该任务就会被执行

调度总耗时 = 当前时间 - 调度开始时间

得出结论: 发生中断后, 如果`taskQueue`中有更高优先级的任务待执行, 之前已完成的工作就会丢失
